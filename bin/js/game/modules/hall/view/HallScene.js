...
class HallScene extends ui.hall.HallSceneUI {
    ...
    init() {
        ...
        if (systemInfo.checkVersion(WXSDKVersion.CREATE_FEEDBACK_BUTTON)) {
            if (self.btnFeedback) {
                let rect = LayerManager.getRealStageRect(self.btnFeedback);
        ...
        //启动游戏出怪
        let startCreateMonster = function () {
        ...
        this.showMenu(false);
        platform.showUpdateVersionTips(() => {
            MessageUtils.showMsgTips("游戏新版本更新失败！");
        });
    }
    /** 初始化用户数据 */
    initUserData() {
        ...
        if (userData) {
            ...
            self.frameOnce(50, self, () => {
                ...
                if (userData) {
                    let remainingTime = userData.cache.getCache(CacheKey.ACCELERATE_SEC_REMAINING);
                    if (remainingTime > 0) {
                                ...
                                if (element) {
                                    ...
                                    if (heroItem && ObjectUtils.isHit(heroItem) && heroItem != that.curMonsterSprite) {
                                        if (!heroItem.isBox() && !heroItem.isLock()) {
                                            ...
                                            let currHeroLevel = BattleManager.Instance.getLevel(heroId);
    ...
    //怪物储存箱
    onCarStore() {
        let store = M.hall.hallData.monsterStore;
    ...
    playAcceEffectView(acceTime = 120, isEffect = true) {
        ...
        HallManager.Instance.hallData.userAcceTime += acceTime;
        if (isEffect) {
            let boneName = userData.isEvolution() ? "fennu2" : "fennu1";
            let bone = new BoneAnim(boneName);
            bone.completeBack = () => {
                bone.destroy();
            };
