class MaskLayer extends Layer {
    ...
    public removeChild(node: Laya.Node): Laya.Node {
        ...
        const sp:Laya.Sprite = node as Laya.Sprite;
        if(sp){
            if(sp.layer_tween){
    ...
    protected superAddChild(node: Laya.Node, index?:number):Laya.Node {
        if(this._useAnimation && !this._animationComplete){
            ...
            if(sp && !sp.layer_tween){
                ...
                const comp:Laya.Component = sp as Laya.Component;
                const size:Laya.Point = new Laya.Point();
                if(comp){
                    size.setTo(comp.displayWidth, comp.displayHeight);
                } else {
                    const rect:Laya.Rectangle = sp.getBounds();
