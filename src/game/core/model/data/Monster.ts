...
class MonsterSprite extends Laya.Sprite {
    ...
    public setAlpha(alphaNum: number): void {
        if (this.isLock() || this.monsterStage == 0) return;
        let roleAni = <Laya.Animation>this.getChildByName(MonsterSprite.roleKey);
        if (roleAni) {
    ...
    dropBoxEffect(_parentNode: any): void {
        ...
        let img: Laya.Image = new Laya.Image("images/hall/drop_box.png");
        img.pivot(30, 30);
        img.scale(0, 0);
        let starPos: Laya.Point = PointUtils.localToGlobal(HallManager.Instance.hall.imgTrain);
