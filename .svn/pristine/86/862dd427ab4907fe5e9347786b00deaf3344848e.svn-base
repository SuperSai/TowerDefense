...
var GameView = /** @class */ (function (_super) {
    ...
    GameView.prototype.initUserData = function () {
        ...
        if (userData) {
            ...
            //分享礼包红点
            if (userData.isShowShareGiftRedPoint()) {
                self.showShareGiftRedPoint();
            }
            //每日签到红点
    ...
    GameView.prototype.addEvents = function () {
        ...
        self.btnShareGift.on(Laya.Event.CLICK, self, self.showShareGiftView);
        self.btnLuckPrize.on(Laya.Event.CLICK, self, self.showLuckPrizeView);
        self.btnFollow.on(Laya.Event.CLICK, self, self.showOfficialAccountView);
        if (self.btnTask)
        ...
        eventCenter.on(shareGiftRedPointEvt, self, self.onUpdateRedPoint); //分享红点移除事件
        ...
        eventCenter.on(followRedPointEvt, self, self.onUpdateFollowRenPoint); //关注红点移除事件
    ...
    /** 更新关注红点 */
    GameView.prototype.onUpdateFollowRenPoint = function ($data) {
        var self = this;
        if ($data == "show") {
            self.showFollowRedPoint();
        }
        else {
            self.showFollowRedPoint(false);
        }
    };
    /** 更新转盘红点 */
    ...
    /** 更新分享红点 */
    GameView.prototype.onUpdateRedPoint = function ($data) {
        var self = this;
        if ($data == "show") {
            self.showShareGiftRedPoint();
        }
        else {
            self.showShareGiftRedPoint(false);
        }
    };
    /** 更新签到红点 */
    ...
    //播放背景音乐
    GameView.prototype.onPlayMusic = function () {
        var that = this;
        Laya.SoundManager.playMusic("musics/bgmusic.mp3", 0);
    };
    //显示超越好友视窗
    ...
    //拖拉菜单
    GameView.prototype.initDragMenu = function () {
        var that = this;
        var curSelectedNode = null;
        var music_switch = "music_switch";
        var musicVolume = Laya.SoundManager.musicVolume;
        var storage = window.localStorage;
        var musicVolumeStr = storage.getItem(music_switch);
        if (musicVolumeStr && musicVolumeStr.length > 0) {
            musicVolume = parseInt(musicVolumeStr);
            if (that.settingView) {
                var menuMusicBtn = that.settingView.getChildByName('menuMusicBtn');
                if (menuMusicBtn) {
                    if (musicVolume > 0) {
                        menuMusicBtn.skin = "images/home_menu_music1.png";
                    }
                    else {
                        menuMusicBtn.skin = "images/home_menu_music2.png";
                    }
                }
                Laya.SoundManager.setMusicVolume(musicVolume);
                Laya.SoundManager.setSoundVolume(musicVolume);
            }
        }
        that.mainView.on(Laya.Event.MOUSE_DOWN, that, function (e) {
            ...
            if (that.settingView) {
                var menuBtn = that.settingView.getChildByName('menuBtn');
                if (menuBtn && CommonFun.isHit(menuBtn)) {
                    curSelectedNode = menuBtn;
                }
                var menuServerBtn = that.settingView.getChildByName('menuServerBtn');
                if (menuServerBtn && CommonFun.isHit(menuServerBtn)) {
                    curSelectedNode = menuServerBtn;
                }
                var menuMusicBtn = that.settingView.getChildByName('menuMusicBtn');
                if (menuMusicBtn && CommonFun.isHit(menuMusicBtn)) {
                    curSelectedNode = menuMusicBtn;
                }
                //点击空白自动回缩
                if (curSelectedNode == null) {
                    if (that.settingView.x >= 0) {
                        that.settingView.x = -70;
                        menuBtn.rotation = 0;
                    }
                }
            }
        ...
        that.mainView.on(Laya.Event.MOUSE_UP, that, function (e) {
            ...
            if (that.settingView) {
                var menuBtn = that.settingView.getChildByName('menuBtn');
                if (menuBtn && CommonFun.isHit(menuBtn) && menuBtn == curSelectedNode) {
                    if (that.settingView.x < 0) { //打开
                        that.settingView.x = 0;
                        menuBtn.rotation = -180;
                    }
                    else { //关闭
                        that.settingView.x = -70;
                        menuBtn.rotation = 0;
                ...
                var menuServerBtn = that.settingView.getChildByName('menuServerBtn');
                if (menuServerBtn && CommonFun.isHit(menuServerBtn) && menuServerBtn == curSelectedNode) {
                ...
                var menuMusicBtn = that.settingView.getChildByName('menuMusicBtn');
                if (menuMusicBtn && CommonFun.isHit(menuMusicBtn) && menuMusicBtn == curSelectedNode) {
                    if (musicVolume > 0) {
                        musicVolume = 0;
                        menuMusicBtn.skin = "images/home_menu_music2.png";
                    }
                    ...
                    Laya.SoundManager.setMusicVolume(musicVolume);
                    Laya.SoundManager.setSoundVolume(musicVolume);
                    console.log("@FREEMAN: 本地数据保存追踪 - music_switch");
                    storage.setItem(music_switch, musicVolume.toString());
                }
            }
        });
    };
    ...
    GameView.prototype.onDiamondBuyAcce = function () {
        ...
        var carPrice = 60;
        //钻石加速超过5次处理
        var acceTimes = userData.diamondAcceTimes() - 5;
        if (acceTimes > 0) {
    ...
    //显示分享礼包红点
    GameView.prototype.showShareGiftRedPoint = function (_show) {
        if (_show === void 0) { _show = true; }
        var that = this;
        if (that.btnShareGift) {
            var imgRedPoint = that.btnShareGift.getChildByName("imgRedPoint");
            if (imgRedPoint) {
                imgRedPoint.visible = _show;
            }
        }
    ...
    //显示关注红点
    GameView.prototype.showFollowRedPoint = function (_show) {
        if (_show === void 0) { _show = true; }
        var that = this;
        if (that.btnLuckPrize) {
            var imgRedPoint = that.btnFollow.getChildByName("imgRedPoint");
            if (imgRedPoint) {
                imgRedPoint.visible = _show;
            }
        }
                ...
                if (that.parkMonsterModelSp && that.curMonsterSprite) {
                                                ...
                                                if (carParkSp.isMaxLevel()) {
                                                    ...
                                                    if (userData.updateCarLevel(CommonFun.getLevel(nextCardId))) {
                                                        if (NoviceManager.cache.checkPetSynthesisLevel) {
                                                            NoviceManager.cache.checkPetSynthesisLevel(userData.getCarLevel());
                                                        }
                                                        //显示红点
                    ...
                    if (CommonFun.isHit(that.curMonsterSprite)) {
                        if (that.curMonsterSprite.isRunning()) {
                            that.curMonsterSprite.setStage(1);
                            //刷新跑道怪物数量
                            that.setRuncarCount(HallManager.Instance.hallData.userRunMonsterCount - 1);
                            //本地保存
                            userData.setCarparkSave(that.curMonsterSprite);
                        }
    ...
    GameView.prototype.setRuncarCount = function (_value) {
        HallManager.Instance.hallData.userRunMonsterCount = _value;
        this.refreshRuncarCount();
        HallManager.Instance.updateIncomePerSec(HallManager.Instance.getCalculateIncomePerSec(this.carparkList));
    };
    GameView.prototype.setRuncarCountMax = function (_value) {
        HallManager.Instance.hallData.userRunMonsterCountMax = _value;
        this.refreshRuncarCount();
    };
    //设置兵营高亮状态
    ...
    GameView.prototype.refreshRuncarCount = function () {
        var that = this;
        if (HallManager.Instance.hallData.userRunMonsterCount < HallManager.Instance.hallData.userRunMonsterCountMax) {
            HallManager.Instance.hallData.userExtraIncome = 1;
            if (that.txtRuncarCount) {
                that.txtRuncarCount.text = HallManager.Instance.hallData.userRunMonsterCount + "/" + HallManager.Instance.hallData.userRunMonsterCountMax;
            }
        }
        else {
            HallManager.Instance.hallData.userExtraIncome = 1.1;
            if (that.txtRuncarCount) {
                that.txtRuncarCount.text = "+10%";
            }
        }
        //跑道已满效果
        if (that.imgCarNum) {
            if ((HallManager.Instance.hallData.userRunMonsterCount < HallManager.Instance.hallData.userRunMonsterCountMax)) {
                that.imgCarNum.skin = "images/game_running_num1.png";
                if (that.txtRuncarCount) {
                    that.txtRuncarCount.y = 3;
                    that.txtRuncarCount.color = "#000000";
                }
                //停止缩放
                if (that.imgCarNumTimeLine) {
                    that.imgCarNumTimeLine.pause();
                    that.imgCarNum.scale(1, 1);
                }
            }
            else {
                that.imgCarNum.skin = "images/game_running_num2.png";
                if (that.txtRuncarCount) {
                    that.txtRuncarCount.y = 25;
                    that.txtRuncarCount.color = "#b11a1a";
                }
                //开始缩放动画
                if (that.imgCarNumTimeLine == null) {
                    that.imgCarNumTimeLine = new Laya.TimeLine();
                    that.imgCarNumTimeLine.addLabel("scale1", 0).to(that.imgCarNum, { scaleX: 0.9, scaleY: 0.9 }, 300)
                        .addLabel("scale2", 100).to(that.imgCarNum, { scaleX: 1, scaleY: 1 }, 300);
                }
                that.imgCarNumTimeLine.play(0, true);
            }
            var imgIcon = that.imgCarNum.getChildByName('imgIcon');
            if (imgIcon) {
                imgIcon.visible = !(HallManager.Instance.hallData.userRunMonsterCount < HallManager.Instance.hallData.userRunMonsterCountMax);
                imgIcon.x = 92;
                imgIcon.y = 21;
                that.imgCarNum.alpha = 0.9;
            }
        }
    };
    /** 赠送宠物中 */
    ...
    //公众号界面
    GameView.prototype.showOfficialAccountView = function () {
        var that = this;
        CommonFun.closeBannerAd(true);
        OfficialAccountView.Create(this, function () {
            //返回
            CommonFun.showBannerAd(true, that.mainView.y);
        });
        //移除红点
    ...
    //分享礼包界面
    GameView.prototype.showShareGiftView = function () {
        var that = this;
        CommonFun.closeBannerAd(true);
        ShareGiftView.Create(LayerManager.getInstance().frameLayer, function () {
            //返回
            CommonFun.showBannerAd(true, that.mainView.y);
        });
    };
    ...
    GameView.prototype.playKingUpdateEffect = function () {
        ...
        var monsterAni = new Laya.Animation();
