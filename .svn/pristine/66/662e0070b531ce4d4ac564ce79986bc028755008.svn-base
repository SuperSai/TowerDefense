...
class EffectUtils extends Laya.Sprite {
    ...
    static showTrainingTimeEffect(parentNode: Laya.Node): void {
        let self = this;
        let img: Laya.Image = new Laya.Image("images/gameLastTimeTip.png");
    ...
    static tipsLabelByObject(obj: Laya.Sprite, content: string, type: number = -1): void {
    ...
    /** 血量特效 */
    static playBloodTextEffect(_parentNode: any, _content: string, _pos: Laya.Point = null, _isDoubleHurt: boolean = false): void {
        //飘文字
        if (Math.random() < 0.6) return;
        let bloodClip: Laya.FontClip = ObjectPool.pop(Laya.FontClip, "BloodFontClip");
    ...
    /** 图片+文本上飘的特效 */
    static playImageTextEffect(_parentNode: any, _imgUrl: string, _content: string, _pos: Laya.Point = null, _zOrder: number = 0): void {
        //图片
