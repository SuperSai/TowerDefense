            ...
            that.requestPrize(day, (_res) => {
                if (_res) {
                    if (_res.code === 1) {
                        ...
                        EffectUtils.tipsLabelByObject(that.btnGet, " +" + DaySignView.signData.prize['day_' + day]["diamond"], EFFECT_TYPE.DIAMOND);
                        const essenceNum: number = DaySignView.signData.prize['day_' + day]["essence"];
                        if (essenceNum) {
                            Laya.timer.once(Time.SEC_IN_MILI, this, () => {
                                EffectUtils.tipsLabelByObject(that.btnGet, " +" + essenceNum, EFFECT_TYPE.ESSENCE);
                            })
                        }
