...
* TER0807-分享礼包;
*/
var ShareGiftView = /** @class */ (function (_super) {
    __extends(ShareGiftView, _super);
    function ShareGiftView(_isTask) {
    ...
    //新建并添加到节点
    ShareGiftView.Create = function (_parentNode, _callback, _isTask) {
        ...
        Laya.loader.load(resList, Handler.create(null, function () {
            var viewTag = _isTask ? "questView" : "shareGiftView";
            if (_parentNode) {
                ...
                var nodeView = new ShareGiftView(_isTask);
    ...
    //初始化
    ShareGiftView.prototype.init = function (_isTask) {
        var that = this;
        //按钮事件
    ...
    ShareGiftView.prototype.onClickExit = function () {
        this.removeSelf();
    };
    ShareGiftView.prototype.onTabChange = function (selectedIndex) {
    ...
    ShareGiftView.prototype.updateTabRetDot = function (subViewType, isShow) {
        var redDot = this._tabGroup.getButtonByIndex(subViewType).getChildByName("imgRetDotHint");
        redDot && (redDot.visible = isShow);
    };
    //初始化list
    //任务
    ShareGiftView.prototype.refreshTaskList = function (_data) {
        var _this = this;
        if (_data == null) {
            return;
        }
                            ...
                            that.requestTaskPrize(_item.id, function (_res) {
                                if (_res) {
                                    if (_res.code === 1) {
                                        ...
                                        if (eventCenter) {
                                            eventCenter.event(diamondChangeEvt, _res);
                                        }
    ...
    //分享
    ShareGiftView.prototype.refreshShareList = function (_data) {
        var _this = this;
        if (_data == null) {
            return;
        }
        ...
        that.shareItemList.renderHandler = new Laya.Handler(that, function (cell, index) {
            ...
            if (imgHead) {
                ...
                imgHead.on(Laya.Event.CLICK, imgHead, function () {
                    CommonFun.lockBtnStage(imgHead);
                ...
                if (boxStage > 0) {
                    ...
                    if (btnInvite) {
                        ...
                        btnInvite.on(Laya.Event.CLICK, btnInvite, function () {
                            CommonFun.lockBtnStage(btnInvite);
                            userData.toShareAd(null, 0, true);
                        });
                    }
                }
    ...
    //拉取任务信息
    ShareGiftView.prototype.requestTaskInfo = function (_callback) {
        var that = this;
    ...
    //拉取奖励
    ShareGiftView.prototype.requestTaskPrize = function (_itemId, _callback) {
        var that = this;
        var HttpReqHelper = new HttpRequestHelper(AppUrl);
        HttpReqHelper.request({
    ...
    //拉取分享信息
    ShareGiftView.prototype.requestShareInfo = function (_callback) {
        var that = this;
        var HttpReqHelper = new HttpRequestHelper(AppUrl);
        HttpReqHelper.request({
    ...
    //拉取奖励
    ShareGiftView.prototype.requestPrize = function (_itemId, _callback) {
        var HttpReqHelper = new HttpRequestHelper(AppUrl);
        HttpReqHelper.request({
    ...
    ShareGiftView.questData = null;
    ShareGiftView.inviteData = null;
...
//# sourceMappingURL=ShareGiftView.js.map