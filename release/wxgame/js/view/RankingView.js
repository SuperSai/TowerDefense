...
var RankingView = /** @class */ (function (_super) {
    ...
    RankingView.prototype.openWorldRankingView = function (_data) {
        ...
        if (_data) {
            _data.forEach(function (element) {
                var asset = CommonFun.parseStringNum(element.stage);
                if (asset > 0) {
        ...
        that.worldRankingList.renderHandler = new Laya.Handler(that, function (cell, index) {
            ...
            if (item) {
                ...
                if (headIcon) {
                    headIcon.skin = item.avatar_url;
                }
                var txtName = cell.getChildByName('txtName');
                if (txtName) {
                    txtName.text = StringTools.omitStringByByteLen(platform.decode(item.nick_name));
                }
                // let txtName = cell.getChildByName('richtextName') as Laya.HTMLDivElement;
                // if (txtName) {
                //     let nickName:string = platform.decode(item.nick_name);
                //     txtName.style.valign = "middle";
                //     txtName.style.wordWrap = true;
                //     txtName.innerHTML = "<span style='color:#8b2811;fontSize:30;overflow:hidden;text-overflow:ellipsis;'>" +nickName +"</span>";
                // }
                ...
                if (txtScore) {
                    var asset = CommonFun.parseStringNum(item.stage);
                    if (asset < 0) {
                        asset = 0;
                    }
                    txtScore.changeText(CommonFun.bytesToSize(asset));
                }
            }
        });
    ...
    RankingView.prototype.showMyRankingView = function (_ranking, _stage) {
        ...
        if (that.viewMyRanking) {
            ...
            if (wxUserInfo) {
                var headIcon = that.viewMyRanking.getChildByName('headIcon');
                if (headIcon) {
                    headIcon.skin = wxUserInfo.avatarUrl;
                }
            ...
            if (txtScore) {
                txtScore.changeText(CommonFun.bytesToSize(_stage));
            }
        }
    };
    //请求周排行数据
